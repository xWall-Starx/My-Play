...
def set_active_hub(hub):
    """Navigate without a browser reload so the Supabase session stays active."""
    hub = str(hub or "home").lower()
    st.session_state["active_hub"] = hub
    main_labels = {
        "home": "Home",
        "play": "Play",
        "course": "Play",
    ...
    player_labels = {
        ...
        "rounds": "Round Biographies",
        "swing": "Swing Video",
        "uploads": "Data Uploads",
    }
    if hub in main_labels:
        st.session_state["main_navigation"] = main_labels[hub]
    if hub in player_labels:
...
def render_route_card(destination, title, description, href):
    ...
    st.markdown(
        f"""
        <section class="myplay-route-card" style="background-image: {background};">
    ...
    route = str(href).split("hub=", 1)[-1].split("&", 1)[0]
    st.button(
        f"Enter {title}",
        type="primary",
        use_container_width=True,
        key=f"route_card_{route}",
...
def render_my_player_room_nav(active):
    ...
    label_to_route = {label: route for route, label in rooms}
    active_label = next(label for route, label in rooms if route == active)
    selected_label = st.pills(
        "My Player rooms",
        list(label_to_route),
        default=active_label,
        key=f"my_player_room_pills_{active}",
        label_visibility="collapsed",
    )
    if selected_label and label_to_route[selected_label] != active:
        set_active_hub(label_to_route[selected_label])
        st.rerun()
...
if "active_hub" not in st.session_state:
    st.session_state["active_hub"] = str(
        st.query_params.get("hub", "home") or "home"
    ).lower()
header_hub = str(st.session_state.get("active_hub", "home") or "home").lower()
if header_hub != "home":
    st.button(
        "← Back to Clubhouse",
        key="back_to_clubhouse",
        on_click=set_active_hub,
        args=("home",),
    )
